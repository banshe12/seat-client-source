...
public class SliderComponent extends AbstractSettingComponent {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Fonts.getSize(20, GUIICONS).drawString(matrix, "H", x + 6, y + 14.5f, new Color(225, 225, 255, 100).getRGB());

        float offset = 62;
        if (nameWidth > 62) {
            ScissorAssist scissor = Rich.getInstance().getScissorManager();
            scissor.push(matrix.peek().getPositionMatrix(), x + 19, y + 12f, 65, 50);
            Fonts.getSize(12, DEFAULT).drawStringWithScroll(matrix, setting.getName(), x + 19, y + 15f, offset, new Color(225, 225, 255, 225).getRGB());
            scissor.pop();

        } else {
            Fonts.getSize(12, DEFAULT).drawString(matrix, setting.getName(), x + 19, y + 15f, 0xFFE1E1E1);
        }
    }
    ...
    private float getDifference(int mouseX, MatrixStack matrix) {
        ...
        rectangle.render(ShapeProperties.create(matrix, v - 10.5f, y + 12.5F, 7, 7)
                .round(3.5f).color(new Color(138, 43, 226, 255).getRGB()).build());

        rectangle.render(ShapeProperties.create(matrix, v - 10F, y + 13.5F, 6, 6)
                .round(3).thickness(2).softness(0).color(new Color(225, 225, 255, 255).getRGB()).build());
